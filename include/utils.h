#ifndef ORANGESQL_UTILS_H
#define ORANGESQL_UTILS_H
...
namespace file_utils {
    ...
    inline size_t size(const std::string& path) {
...
namespace math_utils {
    ...
    inline double percentage(double value, double total) {
        if (total == 0) return 0;
...
namespace base64 {
    ...
    inline std::string encode(const std::vector<uint8_t>& data) {
        ...
        for (uint8_t byte : data) {
            ...
            while (bits >= 6) {
                bits -= 6;
                result += b64_table[(temp >> bits) & 0x3F];
            }
        }
        
        if (bits > 0) {
            result += b64_table[(temp << (6 - bits)) & 0x3F];
        }
    ...
    inline std::vector<uint8_t> decode(const std::string& str) {
...
namespace crc32 {
    ...
    inline uint32_t calculate(const void* data, size_t size, uint32_t crc = 0) {
        crc = ~crc;
    ...
    inline uint32_t calculate(const std::string& str, uint32_t crc = 0) {
        return calculate(str.data(), str.size(), crc);
    }
    
    inline uint32_t calculate(const std::vector<uint8_t>& data, uint32_t crc = 0) {
...
namespace uuid {
    ...
    inline bool isValid(const std::string& uuid) {
...
#define ORANGESQL_DEFER_1(x, y) x##y
#define ORANGESQL_DEFER_2(x, y) ORANGESQL_DEFER_1(x, y)
#define ORANGESQL_DEFER(code) \
    auto ORANGESQL_DEFER_2(_defer_, __LINE__) = Defer([&]() { code; })
